...
class Logger_CUBS:
    ...
    def __init__(
            self, iteration, checkpoint_path, tb_path, output_path, train_loader, val_loader, n_classes, device=None
    ):
        """
    ...
    def end_epoch(
            self, model, multi_label=False, track_explainer_loss=False, save_model_wrt_g_performance=False,
            model_type="g"
    ):
        """
        ...
        if not multi_label:
            ...
            # for multilabel classification
            print(self.epoch_id_total_train_correct)
        ...
        torch.save(
            model.state_dict(), os.path.join(self.checkpoint_path, f"model_seq_epoch_{self.epoch_id}.pth.tar")
        )

        if save_model_wrt_g_performance:
            self.save_model_g(model, model_type)
    ...
    def track_g_loss_stats(self):
        ...
        self.tb.add_scalar(
            "Epoch_stats_g/Accuracy_Correctly_Selected (pi >= 0.5)", self.val_g_accuracy, self.epoch_id
        )
        self.tb.add_scalar(
            "Epoch_stats_g/Accuracy_Correctly_Rejected (pi < 0.5)", self.val_g_incorrect_accuracy, self.epoch_id
        )
    ...
    def save_model_g(self, model, model_type):
        if self.val_g_accuracy > self.best_val_g_accuracy:
            ...
            print(f"\n Old best val accuracy of {model_type} : {self.best_val_g_accuracy} (%) || "
                  f"New best val accuracy of {model_type}: {self.val_g_accuracy} (%) , and new model saved..\n")
    ...
    def save_model(self, model):
        if self.val_accuracy > self.best_val_accuracy:
            chkpt_path = os.path.join(self.checkpoint_path, f"g_best_model_epoch_{self.epoch_id}.pth.tar")
            torch.save(
                model.state_dict(),
