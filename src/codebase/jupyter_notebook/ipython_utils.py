...
def cal_acc(out_put_class_pred, out_put_class_bb_pred, out_put_target, mask_by_pi):
    ...
    t = float(
        torch.where(
            h_rjc == t_rjc, torch.ones_like(h_rjc), torch.zeros_like(h_rjc)
        ).sum()
    )
    f = float(
        torch.where(
    ...
    t_rjc = torch.masked_select(out_put_target, mask_by_pi.bool())
    g_auroc, g_aurpc = utils.compute_AUC(gt=t_rjc, pred=proba[:, 1])
    recall = utils.cal_recall_multiclass(label=t_rjc.numpy(), out=proba.argmax(dim=1).numpy())
    ...
    bb_auroc, bb_aurpc = utils.compute_AUC(gt=t_rjc, pred=proba[:, 1])
    ...
    return acc, g_auroc, bb_auroc, n_rejected, n_selected, coverage, recall
...
def show_results(
    ...
    target_class = g_selected_target[idx].to(torch.int32)
    y_hat = g_selected_pred[idx].argmax(dim=0)
    print("----------------------------")
    print(f"y_gt: {target_class}")
    print(f"y_hat: {y_hat}")
    print(f"class_label: {labels[target_class]}")
    print("----------------------------")
    ...
    while True:
        ...
        imp_concept_vector = test_concepts[idx] * mask_alpha_norm
        if 60 in imp_concept_vector:
            print("concept 60 skipped")
            continue
        y_pred_ex, _, _ = model(imp_concept_vector.unsqueeze(0).to(device))
        y_pred_ex = torch.nn.Softmax()(y_pred_ex).argmax(dim=1)
        print("------------------------------")
    ...
    #     "im": im,
    #     "test_tensor_concepts": test_concepts[idx],
