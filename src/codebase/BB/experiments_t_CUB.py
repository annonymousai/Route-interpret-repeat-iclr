...
def train_t(args):
    ...
    if args.arch == "ResNet50" and args.projected == "y":
        chk_pt_path_bb = os.path.join(args.checkpoints, args.dataset, "explainer", args.arch, args.bb_projected)
        chk_pt_path_t = os.path.join(
            args.checkpoints, args.dataset, "explainer", args.arch, args.root_explainer
        )
        tb_logs_path_t = os.path.join(
            args.logs, args.dataset, "explainer", args.arch, args.root_explainer
        )
        output_path = os.path.join(
            args.output, args.dataset, "explainer", args.arch, args.root_explainer
        )
    elif args.arch == "ResNet50" and args.projected == "n":
    ...
    if args.spurious_waterbird_landbird == "y":
        train_loader, _, val_loader = utils_dataset.get_dataloader_spurious_waterbird_landbird(
            args
        )
    else:
...
def test_t(args):
    ...
    if args.arch == "ResNet50" and args.projected == "y":
        ...
        output_path_t_stats = os.path.join(
            args.output, args.dataset, "explainer", args.arch, args.root_explainer, "stats"
        )
        output_path_t_dataset_g = os.path.join(
            args.output, args.dataset, "explainer", args.arch, args.root_explainer, "dataset_g"
        )
    elif args.arch == "ResNet50" and args.projected == "n":
        root = f"lr_{args.lr}_epochs_{args.epochs}"
        chk_pt_path_bb = os.path.join(args.checkpoints, args.dataset, "BB", root, args.arch)
    ...
    start = time.time()
    if args.spurious_waterbird_landbird == "y":
        train_loader, val_loader, test_loader = utils_dataset.get_dataloader_spurious_waterbird_landbird(
            args
        )
    else:
        transforms = utils.get_train_val_transforms(args.dataset, args.img_size, args.arch)
    ...
    with torch.no_grad():
        with tqdm(total=len(loader)) as t:
            for batch_id, data_tuple in enumerate(loader):
                ...
                feature_x = get_phi_x(image, bb, arch, layer)
                scale = len(loader.dataset) / feature_x.size(0)
                concept_T = attribute[:, 108: 110]
                I = torch.eye(feature_x.size(0)).to(device)
                x = concept_T
                x_T = torch.transpose(concept_T, 0, 1)
                phi_r_x = torch.mm(
                    I - scale * torch.mm(x, torch.mm(sigma_train, x_T)),
                    feature_x.reshape(image.size(0), -1)
                ).reshape(-1, feature_x.size(1), feature_x.size(2), feature_x.size(3))
                logits_concepts = t_model(phi_r_x)
                # logits_concepts = t_model(feature_x)
                y_hat = torch.sigmoid(logits_concepts)
