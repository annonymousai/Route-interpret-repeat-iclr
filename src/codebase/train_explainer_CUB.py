...
from Explainer.experiments_explainer_CUB import train_glt, train_baseline_post_hoc
...
def main():
    ...
    if args.spurious_specific_classes == "y":
        ...
        for arg in vars(args):
            print(f"{arg}: {getattr(args, arg)}")
        train_glt_waterbird_landbird(args)

    elif args.train_baseline == "y":
        ...
        for arg in vars(args):
            print(f"{arg}: {getattr(args, arg)}")
        print("Training explainer for CUB")
        train_glt(args)
