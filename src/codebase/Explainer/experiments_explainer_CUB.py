...
import utils
from Explainer.concept_extractor import do_explain_class
...
def extract_concepts(args):
    ...
    root = (
        ...
        f"_temperature-KD_{args.temperature_KD}_hidden-layers_{len(args.hidden_nodes)}"
    ...
    output_path = os.path.join(args.output, args.dataset, "explainer", root)

    output_path_model_outputs = os.path.join(output_path, "model_outputs")
    output_path_g_outputs = os.path.join(output_path, "g_outputs")
    output_path_g_explanations = os.path.join(output_path, "explanations")
    os.makedirs(output_path_g_explanations, exist_ok=True)
    ...
    tensor_alpha = torch.load(
        os.path.join(root, output_path_g_outputs, "test_tensor_alpha.pt")
    )

    tensor_alpha_norm = torch.load(
        os.path.join(root, output_path_g_outputs, "test_tensor_alpha_norm.pt")
    )

    tensor_concept_mask = torch.load(
        os.path.join(root, output_path_g_outputs, "test_tensor_concept_mask.pt")
    )
    ...
    test_tensor_conceptizator_concepts = torch.load(
        os.path.join(root, output_path_g_outputs, "test_tensor_conceptizator_concepts.pt")
    )

    test_tensor_concepts = torch.load(
        os.path.join(root, output_path_g_outputs, "test_tensor_concepts.pt")
    )

    test_tensor_preds = torch.load(
        os.path.join(root, output_path_g_outputs, "test_tensor_preds.pt")
    )

    test_tensor_y = torch.load(os.path.join(root, output_path_g_outputs, "test_tensor_y.pt"))

    # val
    val_tensor_conceptizator_concepts = torch.load(
        os.path.join(root, output_path_g_outputs, "val_tensor_conceptizator_concepts.pt")
    )

    val_tensor_concepts = torch.load(
        os.path.join(root, output_path_g_outputs, "val_tensor_concepts.pt")
    )

    val_tensor_preds = torch.load(
        os.path.join(root, output_path_g_outputs, "val_tensor_preds.pt")
    )

    val_tensor_y = torch.load(os.path.join(root, output_path_g_outputs, "val_tensor_y.pt"))

    # train
    train_tensor_conceptizator_concepts = torch.load(
        os.path.join(root, output_path_g_outputs, "train_tensor_conceptizator_concepts.pt")
    )

    train_tensor_concepts = torch.load(
        os.path.join(root, output_path_g_outputs, "train_tensor_concepts.pt")
    )

    train_tensor_preds = torch.load(
        os.path.join(root, output_path_g_outputs, "train_tensor_preds.pt")
    )

    train_tensor_y = torch.load(os.path.join(root, output_path_g_outputs, "train_tensor_y.pt"))
    ...
    test_tensor_y_1h = F.one_hot(test_tensor_y.to(torch.int64), num_classes=n_classes)
    train_tensor_y_1h = F.one_hot(train_tensor_y.to(torch.int64), num_classes=n_classes)
    val_tensor_y_1h = F.one_hot(val_tensor_y.to(torch.int64), num_classes=n_classes)
    ...
    glt.load_state_dict(torch.load(os.path.join(chk_pt_path, args.checkpoint_model)))
    glt.eval()
    ...
    print(train_tensor_y_1h.size())

    start = time.time()
    module = {
        "alpha": tensor_alpha,
        "alpha_norm": tensor_alpha_norm,
        "concept_mask": tensor_concept_mask,
        "conceptizator_threshold": conceptizator_threshold
    }
    results, explanations_list = do_explain_class(
        ...
        module, glt, concept_names=args.concept_names, top_k_explanations=top_k_explanations, verbose=True
    )
    end = time.time() - start
    results['model_accuracy'] = torch.sum(test_tensor_preds.argmax(dim=1).eq(test_tensor_y)) / test_tensor_y.size(0)
    results['extraction_time'] = end

    extracted_concepts = []
    all_concepts = module["concept_mask"][0] > 0.5
    common_concepts = module["concept_mask"][0] > 0.5
    explanations = {i: [] for i in range(n_classes)}

    for j in range(n_classes):
        n_used_concepts = sum(module["concept_mask"][j] > 0.5)
        print(f"Extracted concepts: {n_used_concepts}")
        print(f"Explanation: {explanations_list[j]['explanation']}")
        print(f"Explanation f1  : {explanations_list[j]['explanation_f1']}")
        print(f"Explanation accuracy: {explanations_list[j]['explanation_accuracy']}")
        if explanations_list[j]['explanation'] is not None:
            explanations[j].append(explanations_list[j]['explanation'])
        extracted_concepts.append(n_used_concepts)
        all_concepts += module["concept_mask"][j] > 0.5
        common_concepts *= module["concept_mask"][j] > 0.5

    results['extracted_concepts'] = np.mean(extracted_concepts)
    results['common_concepts_ratio'] = sum(common_concepts) / sum(all_concepts)

    print(results)
    utils.dump_in_pickle(output_path, "explanations.pkl", results)
    df = pd.DataFrame(explanations_list)
    df.to_csv(os.path.join(output_path, "explanations" + ".csv"))


def train_baseline_post_hoc(args):
    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    ...
    val_dataset = Dataset_cub_for_explainer(
        dataset_path, "test_proba_concepts.pt", "test_class_labels.pt", "test_attributes.pt", test_data, val_transform
    )
    ...
    g_chk_pt_path = os.path.join(args.checkpoints, args.dataset, "Baseline_PostHoc", args.arch, root)
    g_output_path = os.path.join(args.output, args.dataset, "Baseline_PostHoc", args.arch, root)
    g_tb_logs_path = os.path.join(args.logs, args.dataset, "Baseline_PostHoc", args.arch, root)
...
def fit_baseline(
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader)) as t:
            for batch_id, (train_images, train_concepts, train_y, train_y_one_hot) in enumerate(train_loader):
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader)) as t:
                for batch_id, (val_images, val_concepts, val_y, val_y_one_hot) in enumerate(val_loader):
                    ...
                    y_hat = model(val_concepts)
                    entropy_loss_elens = entropy_loss(model)
                    val_loss = criterion(y_hat, val_y) + lambda_lens * entropy_loss_elens
                    logger.track_val_loss(val_loss.item())
                    logger.track_total_val_correct_per_epoch(y_hat, val_y)
                    t.set_postfix(
                        epoch='{0}'.format(epoch + 1),
                        validation_loss='{:05.3f}'.format(logger.epoch_val_loss))
                    t.update()

        logger.end_epoch(model)
...
def test_glt(args):
    ...
    start = time.time()
    transforms = utils.get_train_val_transforms(args.dataset, args.img_size, args.arch)
    train_transform = transforms["save_transform"] if args.expert_to_train == "explainer" \
        else transforms["train_transform"]
    test_transform = transforms["save_transform"] if args.expert_to_train == "explainer" \
        else transforms["val_transform"]
    ...
    train_dataset = Dataset_cub_for_explainer(
        dataset_path,
        "train_proba_concepts.pt",
        "train_class_labels.pt",
        "train_attributes.pt",
        train_data,
        train_transform,
    )
    ...
    test_dataset = Dataset_cub_for_explainer(
        dataset_path,
        "test_proba_concepts.pt",
        "test_class_labels.pt",
        "test_attributes.pt",
        test_data,
        test_transform,
    )
    ...
    if args.expert_to_train == "explainer":
        ...
        test_explainer(args, cov, lr_explainer, root, iteration, use_concepts_as_pi_input, train_loader, val_loader,
                       test_loader)
    elif args.expert_to_train == "residual":
        test_residual(args, root, iteration, use_concepts_as_pi_input, train_loader, val_loader, test_loader)


def test_residual(args, root, iteration, use_concepts_as_pi_input, train_loader, val_loader, test_loader):
    ...
    if iteration == 1:
        ...
        g_output_path = os.path.join(
            args.output, args.dataset, "explainer", args.arch, root, f"iter{iteration}", "explainer"
        )
        residual_output_path = os.path.join(
            args.output, args.dataset, "explainer", args.arch, root, f"iter{iteration}", "bb"
        )
        ...
        g_output_path = os.path.join(
            args.output, args.dataset, "explainer", args.arch, root, f"cov_{cov}_lr_{lr_explainer}", f"iter{iteration}",
            "explainer"
        )
        residual_output_path = os.path.join(
            args.output, args.dataset, "explainer", args.arch, root, f"cov_{cov}_lr_{lr_explainer}", f"iter{iteration}",
    ...
    print(f"===> Latest glt checkpoint is loaded for iteration {iteration}: {cur_glt_chkpt}")
    glt = Gated_Logic_Net(
        args.input_size_pi,
        args.concept_names,
        args.labels,
...
def save_results_selected_residual_by_pi(
    ...
    with torch.no_grad():
        with tqdm(total=len(loader)) as t:
            for batch_id, (images, concepts, _, y, y_one_hot) in enumerate(loader):
...
def predict_residual(
    ...
    with torch.no_grad():
        with tqdm(total=len(test_loader)) as t:
            for batch_id, (test_images, test_concepts, _, test_y, test_y_one_hot) in enumerate(test_loader):
                ...
                with torch.no_grad():
                    bb_logits = bb(test_images)
                    test_feature_x = bb.feature_store[layer]
...
def test_explainer(
        args, cov, lr_explainer, root, iteration, use_concepts_as_pi_input, train_loader, val_loader, test_loader
):
    ...
    print(f"==>> Latest G for iteration {iteration} is loaded from {glt_chk_pt}")
...
def save_results_selected_by_pi(
    ...
    with torch.no_grad():
        with tqdm(total=len(data_loader)) as t:
            for batch_id, (images, concepts, attributes, y, y_one_hot) in enumerate(
                    data_loader
            ):
...
def predict(
    ...
    with torch.no_grad():
        with tqdm(total=len(test_loader)) as t:
            for batch_id, (test_images, test_concepts, _, test_y, test_y_one_hot) in enumerate(
                    test_loader
            ):
...
def train_glt(args):
    ...
    # print("Training G")
    if args.expert_to_train == "explainer":
        train_explainer(args, cov, lr_explainer, root, iteration, use_concepts_as_pi_input, train_loader, val_loader)
    elif args.expert_to_train == "residual":
        train_residual(args, root, iteration, use_concepts_as_pi_input, train_loader, val_loader)


def train_residual(args, root, iteration, use_concepts_as_pi_input, train_loader, val_loader):
    chk_pt_explainer = os.path.join(args.checkpoints, args.dataset, "explainer", args.arch, root)
    chk_pt_residual = os.path.join(args.checkpoints, args.dataset, "explainer", args.arch, root)
    if iteration == 1:
        g_chk_pt_path = os.path.join(chk_pt_explainer, f"iter{iteration}", "explainer")
    ...
    residual_tb_logs_path = os.path.join(args.logs, args.dataset, "explainer")
...
def train_explainer(args, cov, lr_explainer, root, iteration, use_concepts_as_pi_input, train_loader, val_loader):
    chk_pt_explainer = os.path.join(args.checkpoints, args.dataset, "explainer", args.arch, root)
    if iteration == 1:
        g_chk_pt_path = os.path.join(chk_pt_explainer, f"iter{iteration}", "explainer")
        g_output_path = os.path.join(args.output, args.dataset, "explainer", args.arch, root, f"iter{iteration}",
                                     "explainer")
        g_tb_logs_path = os.path.join(args.logs, args.dataset, "explainer", f"iter{iteration}", args.arch, root)
    else:
        ...
        g_tb_logs_path = os.path.join(
            args.logs, args.dataset, "explainer", f"iter{iteration}", args.arch,
            f"cov_{cov}_lr_{lr_explainer}-explainer"
        )
